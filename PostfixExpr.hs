...
foldStep stack token =
       case token of
         "+"  -> Add (head (tail stack)) (head stack)  : tail (tail stack)
         "-"  -> Sub (head (tail stack)) (head stack) : tail (tail stack)
         "*"  -> Mul (head (tail stack)) (head stack)  : tail (tail stack)
         "uminus" -> Uminus (head stack) : tail stack
         _    -> Leaf (read token) : stack

        -- Main function to parse the PostFixExpr
postfixExpr postfix =
 let tokens = words postfix
 in case foldl foldStep [] tokens of
      [expr] -> expr
