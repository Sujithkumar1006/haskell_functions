...
listMap :: (a -> b -> c) -> a -> [b] -> [c]
listMap _ _ [] = []
listMap num op list = map (num op) list
...
member _ [] = False
member num list = foldl (\acc x -> acc || (x == num)) False list
...
selectNApart _ [] = []
selectNApart num (x:xs) = x : if length xs > (num - 1) then selectNApart num (drop (num - 1) xs) else []
